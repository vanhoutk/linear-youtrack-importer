...
import { replaceImagesInMarkdown } from "./utils/replaceImages.js";
 ...
 */
export const importIssues = async (apiKey, importer) => {
  ...
  if (importAnswers.newTeam) {
    ...
    teamId = importAnswers.targetTeamId;
  }
  ...
  for (const issue of importData.issues) {
    ...
    const existingAssigneeId = !!issue.assigneeId
  ...
  console.info(`${importer.name} issues imported to your team: https://linear.app/team/${teamKey}/all`);
};
